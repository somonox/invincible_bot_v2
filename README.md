...
GUI는 **왼쪽 콤보 우선 · 오른쪽 PC/콤보 자동 전환**입니다. 오른쪽과 온라인 봇은 내 가비지 큐의 양·도착 시점·예상 상쇄량을 보고 PC와 콤보를 선택합니다. 고정 콤보 수 기준은 없으며, 블록 개수로 PC 가능성을 거르는 조건은 유지합니다. 양쪽의 현재 콤보, PC 횟수, 최근 스핀과 공격량을 표시합니다. [자동 전환 정책](docs/HYBRID_POLICY.md)을 참고하세요. 같은 탐색 깊이와 동시 배치를 유지하며, SRS-X 90/180도 킥과 스핀 보너스 선택을 지원합니다. [적용 규칙과 검증 범위](docs/TETRIO_RULES.md), [PC 정책](docs/PERFECT_CLEAR.md)을 참고하세요.
