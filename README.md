...
6. **온라인 방 설정과 리플레이 관리**
   - 봇에게 방장을 넘기고 `!setup`을 입력하면 SRS-X·4×20 등 필수 설정을 적용한 뒤 방장 권한을 돌려줍니다. 설정이 맞을 때까지 관전하며, 마지막 사람이 나가면 바로 퇴장합니다. PPS 상한은 5, 기본 워커는 20개 방이며 각 워커가 독립적으로 탐색합니다. 대국은 기간·개수·용량 제한 없이 로컬 `.ttrm`으로 저장합니다. [운영 정책](docs/SERVICE_POLICY.md)을 참고하세요.
