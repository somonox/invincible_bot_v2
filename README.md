...
GUI는 **왼쪽 콤보 우선 · 오른쪽 PC/콤보 자동 전환**입니다. 오른쪽은 상대 20콤보 미만이면 PC를 노리고, 상대 콤보가 높거나 PC가 불가능하거나 미리보기에서 PC를 못 찾으면 콤보로 전환합니다. 전환 기준은 GUI에서 조절합니다. 양쪽의 현재 콤보, PC 횟수, 최근 스핀과 공격량을 표시합니다. [자동 전환 정책](docs/HYBRID_POLICY.md)을 참고하세요. 같은 탐색 깊이와 동시 배치를 유지하며, SRS-X 90/180도 킥과 스핀 보너스 선택을 지원합니다. [적용 규칙과 검증 범위](docs/TETRIO_RULES.md), [PC 정책](docs/PERFECT_CLEAR.md)을 참고하세요.
