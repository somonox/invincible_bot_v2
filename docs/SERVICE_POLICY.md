...
The online bot requires SRS-X. When it owns the room, it applies compatible
settings; otherwise it spectates and lists the settings the host must change.
Giving the bot host allows automatic configuration. The actual round engine is
...
- CPU search admission is FIFO, limited to at most three simultaneous searches
  (at most available CPUs minus one, minimum one). The queue is bounded by the
  configured worker count. This is a capacity ceiling, not a guarantee that all
  20 rooms can sustain 5 PPS simultaneously.
- Idle lobbies are left after 15 minutes; an ongoing match is not evicted for
  inactivity. Empty rooms are left immediately.
- Connection, adapter initialization, search admission and individual moves have
...
checks typing, settings/host transitions, speed/command permissions, admission
fairness and cancellation, process failures, stale rounds, idle cleanup, replay
